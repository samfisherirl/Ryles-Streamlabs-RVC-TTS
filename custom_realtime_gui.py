...
if __name__ == "__main__":
    ...
    class GUI:
        ...
        def launcher(self):
            ...
            self.window = sg.Window("Ryle's Realtime RVC", layout=layout, finalize=True)
            self.event_handler()
            ...
            while True:
                ...
                if event == '__TIMEOUT__':
                    ...
                    if print_counter > 30:
                        print_counter = 0
                        count_up += 1
                        print(f"Ryle's {event}# {count_up}")
                    try:
                        config_contents = self.check_and_update_config()
                        if config_contents:
                            self.window["pth_path"].update( config_contents['weight'].replace('\\\\', '\\') )
                            self.window["index_path"].update( config_contents['index'].replace('\\\\', '\\') )
                            self.window["pitch"].update(config_contents['pitch'])
                            values['pth_path'] = self.window["pth_path"].get()
                            values['index_path'] = self.window["index_path"].get()
                            self.gui_config.pitch = config_contents['pitch']
                            if firsttime:
                                firsttime = False
                            stream_latency = -1
                            event = 'stop_vc'
                            self.stop_and_start = True
                        else:
                            continue
                    except Exception as e:
                        print(str(e))
                ...
                if event == "start_vc" and self.flag_vc == False:
                    values = self.start_vc(values)
                    
                if event == "stop_vc" and self.flag_vc == True:
                    self.flag_vc = False
                    stream_latency = -1
                    print('\n\n VC stopped\n\n')
                    if self.stop_and_start:
                        self.window['start_vc'].click()
                        self.stop_and_start = False
        ...
        def audio_callback(
            ...
            # Run GUI updates off the main thread
            threading.Thread(target=lambda: self.window["infer_time"].update(int(total_time * 1000))).start()
